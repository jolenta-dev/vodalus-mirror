...
export class journeyTabs {
  ...
  constructor(tabContent: string[] = ["I -- Nessus", "II -- Saltus and beyond", "III -- Thrax", "IV -- Autarchy"]) {
    ...
    this.tabContainer.style.backgroundColor = "#1a0033";
    this.tabContainer.style.color = "aliceblue";
    ...
    for (let i: number = 0; i < tabContent.length; i++) {
      ...
      btn.btn.style.textAlign = "center";
      ...
      btn.btn.style.backgroundColor = "#1a0033";
  ...
  paint(el: HTMLButtonElement): void {
    ...
    if (active) {
      el.style.backgroundColor = hover ? "#dcc2f3" : "#9a7cb6"; // active hover / active
    } else {
      el.style.backgroundColor = hover ? "#dcc2f4" : "#1a0033"; // default hover / default
    }
  }
  ...
  toggleActive(el: HTMLButtonElement): void {
    if (el == this.active) {
      ...
      this.active.style.backgroundColor = "#1a0033";

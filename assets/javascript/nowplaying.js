...
let IDLE_AFTER_SCROBBLE_MS = 15 * 60 * 1e3;
let getSetLastFM = function() {
  fetch("/api/lastfm").then(function(resp) {
    ...
    let recentTrack = tracks.find(function(t) {
      return t["@attr"] && t["@attr"].nowplaying === "true";
    }) || tracks[0];
    ...
    if (isNowPlaying) {
      ...
      let utsMs = utsRaw != null ? parseInt(String(utsRaw), 10) * 1e3 : NaN;

...
fetch('/api/names')
  .then(r => r.json())
  .then(names => names.forEach(({ name, date, website, note, vip, journey_level, prestige_level, color, decoration, holds_clicker_tag }) => {
...
function newName() {
  ...
  if (inputName) {
    // try to login with *no* password first. if protected, prompt for password.
    ...
    tryLogin().then(async r => {
      if (r.status === 401) {
        // protected name, prompt for password and try again
        // TODO: also make sure people can't use the same name multiple times
        ...
        // password correct, proceed to add name
        submitName(pw);
      } else if (!r.ok) {
        const res = await r.json();
        return alert(res.error || 'Login failed.');
      } else {
        // not protected or no password needed
        submitName();
      }
    }).catch(err => {
    ...
    function submitName(password = null) {
      const nameCells = document.querySelectorAll('#name-table-body tr td:nth-child(2)');
      ...
      fetch('/api/names', {
        ...
        .then(async (r) => {
          ...
          addToList(inputName, new Date().toLocaleString(), inputWebsite, inputNote, {
            vip: body.vip,
...
addToList('jolenta', new Date(0), 'https://vodalus.org', 'i was here first', {
  vip: false,
  journey_level: 4,
  prestige_level: 7,
  color: '#c71585',

...
function rosterNameHtml(name, isVip, journeyLevel, color, decoration, prestigeLevel, selectedTag, allTags) {
  ...
  const jolentaFixedRoleTags = n === "jolenta" ? tagToSpanHtml("OWNER", prestigeLevel) : "";
  const jolentaPickerUpper = n === "jolenta" && chosenTag ? String(chosenTag).trim().toUpperCase() : "";
  const selectedTagHtml = n === "jolenta" ? chosenTag && jolentaPickerUpper !== "OWNER" ? tagToSpanHtml(chosenTag, prestigeLevel) : "" : chosenTag ? tagToSpanHtml(chosenTag, prestigeLevel) : "";
  ...
  const tooltipHtml = tags.length ? tags.map((t) => tagToSpanHtml(t, prestigeLevel)).join("") : "<span>(NO TAGS)</span>";
  const displayName = n === "jolenta" ? '<span class="message-name">' + nameText + '</span><span class="message-name-tail" aria-hidden="true">\u02D6<span class="message-name-tail-gold">\u2727</span>\xB0.</span>' : '<span class="message-name">' + nameText + "</span>";
  color = color ?? "#000000";
  let deco = decoration ?? "";
  if (n === "jolenta") deco = "";
  ...
  if (safeDecoration) {
    ...
    decoTrail = '<span class="message-name-deco message-name-deco--mirror" aria-hidden="true" style="display:inline-block;transform:scale(-1,1)">' + safeDecoration + "</span>";
  }
...
let unreadConversationIds = /* @__PURE__ */ new Set();
...
function mergeAnonLastReadIntoAfterNickLoad() {
  ...
  for (const k of Object.keys(anonObj)) {
    if (lastReadMessageCount[k] === void 0) lastReadMessageCount[k] = anonObj[k];
  }
...
function maybeInsertUnreadDivider() {
  ...
  if (prev === void 0 || prev === null) return;
...
function connect() {
  ...
  function openWebSocket(wsAuthToken) {
    ...
    ws.onmessage = (event) => {
      ...
      if (type === "history") {
        ...
        persistUnreadState();
        if (lastReadMessageCount[hid] === void 0) {
  ...
  fetch("/api/ws-auth", { credentials: "include" }).then((r) => r.ok ? r.json() : null).then((body) => openWebSocket(body && body.token)).catch(() => openWebSocket(null));
}
loadLastReadState();
loadUnreadState();
connect();
...
function submitName(loginName, pw = null) {
  nickname = loginName;
  password = pw;
  unreadConversationIds = /* @__PURE__ */ new Set([...getPersistedUnreadIds("__anon__"), ...getPersistedUnreadIds(loginName)]);
...
document.getElementById("join-btn").addEventListener("click", () => {
  ...
  const tryLogin = (password2 = null) => fetch("/api/login", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
function formatChatTimestampForDisplay(raw) {
  ...
  return t.toLocaleTimeString(void 0, { hour: "numeric", minute: "2-digit", second: "2-digit" });
}
...
function addToList(name, message, date, isVip, color, decoration, journey_level = 0, prestige_level = 0, selected_chat_tag = "", all_tags = [], id) {
  ...
  if (lastDay !== dayKey) {
    ...
    sep.textContent = d.toLocaleDateString(void 0, { year: "numeric", month: "long", day: "numeric" });
...
function prependOlderMessages(items) {
  ...
  items.forEach((item) => {
    ...
    if (item.kind === "announcement") {
      ...
      li2.className = "date-separator";
      if (!Number.isNaN(d.getTime())) li2.dataset.date = dayKey;
      li2.textContent = "[" + formatChatTimestampForDisplay(item.date) + "] " + item.name + ": " + item.message;
...
function loadOlderMessages() {
  ...
  fetch("/api/conversations/" + encodeURIComponent(conv) + "/messages?beforeId=" + encodeURIComponent(beforeId) + "&limit=" + HISTORY_PAGE_SIZE, { credentials: "include" }).then((r) => r.ok ? r.json() : Promise.reject(new Error("failed"))).then((payload) => {
    if (conv !== currentConversationId) return;
    const arr = payload && payload.messages || [];
    const anns = payload && payload.announcements || [];
    if (!arr.length && !anns.length) {
      noMoreOlder = true;
      return;
    }
    ...
    arr.forEach((m) => {
      if (m.id != null && (newOldest == null || m.id < newOldest)) newOldest = m.id;
    });
...
function refreshSidebarMemberList() {
  ...
  fetch("/api/conversations/" + encodeURIComponent(currentConversationId) + "/members", { credentials: "include" }).then((r) => {
...
function loadConversations() {
  return fetch("/api/conversations", { credentials: "include" }).then((r) => r.ok ? r.json() : Promise.reject(new Error("failed to load conversations"))).then((data) => {
...
function bindPrivilegedControls() {
  ...
  function refreshChatAdminMembersList() {
    ...
    fetch("/api/conversations/" + encodeURIComponent(currentConversationId) + "/members", { credentials: "include" }).then((r) => r.ok ? r.json() : Promise.reject(new Error("failed to load members"))).then((data) => {
      const arr = data && data.members ? data.members : [];
      if (!arr.length) {
  ...
  chatTagBtn.addEventListener("click", () => {
    fetch("/api/chat-tag-preference", { credentials: "include" }).then(async (r) => {
      const payload = await r.json().catch(() => ({}));
      if (!r.ok) throw new Error(payload.error || "could not load tags");
      return payload;
    }).then((payload) => {
      if (payload.clicker_tag_holder !== void 0) clickerTagHolder = payload.clicker_tag_holder;

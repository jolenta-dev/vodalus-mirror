...
nameInputEl.addEventListener('input', () => { nameInputEl.dataset.touched = '1'; });
...
document.getElementById('board-btn').addEventListener('click', async () => {
  ...
  const meBoard = sessionMatches
    ? meNow
    : await fetch('/api/me', { credentials: 'include', cache: 'no-store' }).then(r => (r.ok ? r.json() : null));
...
document.getElementById('yesod-journey-btn').addEventListener('click', async () => {
  ...
  btn.disabled = true;
  setTimeout(function () { onCooldown = false; btn.disabled = false; }, COOLDOWN_MS);
  var name = document.getElementById('name-input').value.trim();
  ...
  if (stepData.outcome === 'already_won') {
    clearYesodJourneyList();
    yesodFinalWinMessages.forEach(function (line) { addYesodJourneyItem(line); });
    yesodJourneyStage = 4;
    return;
  }
